...
test('createMeme', function(t) {
    t.test('create an image from single source', function(t) {
        ...
        captainApi.createImage('https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png',
            'google logo'
    ...
    t.test('invalid arguments', function(t) {
        ...
        captainApi.createImage('not an url', 'foo').then(function() {
            t.fail('Resolve when there is invalid argument');
        }, function(err) {
            t.equal(/^urls must be an url or an array of urls.*/.test(err.message),
                true,
